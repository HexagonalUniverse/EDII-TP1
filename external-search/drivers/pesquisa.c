...
typedef enum {
    ...
    ES_REG_FILE_WONT_OPEN,              // File won't open when trying to display it's registries
    ES_LOGGING_INITIALIZATION_ERROR,    // Logging-system couldn't initialize properly.
    ES_RUNTIME_ERROR,                   // Had an error while running the searching-engines.
...
static inline bool _isNumeral(const char * const _String)
{
    size_t iterator = 0;
    while (_String[iterator])
    {
        if (nin_range(48, 57, _String[iterator]))
            return false;
        iterator ++;
    }
    return true;
...
_ParseArgs(int argc, char ** argsv, struct application_parameters * parameters)
{
    ...
    if (argc == 6) {
        ...
        } else  parameters -> display_keys = true;
    }

    // Getting the method number from terminal
    if (! _isNumeral(argsv[1])) {
        _ContextErrorMsgf("parsing error: ", "The argument #1 isn't a numeral string: <%s>\n", argsv[1]);
        return false;
    }
...
int main(int argc, char ** argsv)
{
    ...
    if (! InitializeLogging())
    {
        ...
        return ES_LOGGING_INITIALIZATION_ERROR;
    }
